  ...
  if (isTracking) {
    return (
      <div className="min-h-screen bg-gray-50 text-gray-900 flex">
        {/* Sidebar */}
        <div className="w-96 border-r border-gray-200 bg-white p-8">
          <h2 className="text-xl font-light text-gray-900 mb-8 tracking-tight">Active Tracking</h2>
          
          <div className="space-y-6">
            <div className="p-6 bg-gray-50 rounded-2xl border border-gray-200">
              <div className="flex items-center gap-4 mb-3">
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-semibold ${
                  selectedCoin === 'BTC' ? 'bg-orange-50 text-orange-600' :
                  selectedCoin === 'ETH' ? 'bg-blue-50 text-blue-600' :
                  selectedCoin === 'SOL' ? 'bg-purple-50 text-purple-600' :
                  'bg-amber-50 text-amber-600'
                }`}>
                  {selectedCoin}
                </div>
                <div>
                  <div className="text-lg font-medium text-gray-900">{selectedCoin} — {strategy}</div>
        ...
        <div className="flex-1 p-12">
          <div className="max-w-5xl mx-auto space-y-12">
            ...
            <div className="bg-white rounded-2xl p-8 border border-gray-200 shadow-sm">
              <h3 className="text-2xl font-light text-gray-900 mb-6 tracking-tight">Market Summary</h3>
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <span className="text-gray-700 text-lg">Current Price:</span>
                ...
                <div className="flex items-center justify-between">
                  <span className="text-gray-700 text-lg">24h Change:</span>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-gray-600">Every {updateInterval}s</span>
                </div>
              </div>
            </div>
            ...
            {/* Key Signals */}
            <div className="bg-white rounded-2xl p-8 border border-gray-200 shadow-sm">
              <h3 className="text-2xl font-light text-gray-900 mb-8 tracking-tight">Live Signals</h3>
              <div className="space-y-6">
                {marketSignals.length > 0 ? (
                  marketSignals.map((signal, index) => (
                    <div key={index} className="flex items-center gap-4">
                      <div className={`w-3 h-3 rounded-full ${
                        signal.bias === 'BULLISH' ? 'bg-emerald-500' :
                        signal.bias === 'BEARISH' ? 'bg-red-500' :
                        'bg-gray-500'
                      }`}></div>
                      <div className="flex-1">
                        <div className="text-lg font-medium text-gray-900">{signal.type}</div>
                        <div className="text-gray-600">{signal.confidence}% confidence</div>
                      </div>
                    </div>
                  ))
                ) : (
            ...
            {/* Chart Area */}
            <div className="bg-white rounded-2xl p-8 border border-gray-200 shadow-sm">
              <h3 className="text-2xl font-light text-gray-900 mb-8 tracking-tight">Live Price Chart</h3>
